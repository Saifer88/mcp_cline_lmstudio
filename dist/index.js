...
server.tool("web_search", "Search the web using DuckDuckGo. Returns titles, URLs, and snippets for search results.", {
...
server.tool("fetch_webpage", "Fetch a webpage and extract its text content. Useful for reading articles, documentation, or any web page.", {
...
server.tool("download_image", "Download an image from a URL and save it to a local file path.", {
    url: z.string().url().describe("The image URL to download"),
    savePath: z
        .string()
        .describe("Local file path where the image should be saved (e.g. ./images/photo.png)"),
}, async ({ url, savePath }) => {
    try {
...
server.tool("execute_command", "Execute a shell command and return stdout/stderr. Use with caution. Blocked commands: rm -rf /, format, mkfs, dd if=.", {
...
server.tool("read_pdf", "Extract text content from a PDF file (local path or URL).", {
    source: z
        .string()
